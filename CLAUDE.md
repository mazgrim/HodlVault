...
**API prefixes:** `/api/auth`, `/api/portfolios`, `/api/transactions`, `/api/dividends`, `/api/performance`, `/api/market`, `/api/import`, `/api/admin`, `/api/tools`, `/api/benchmark`, `/api/demo`, `/api/coupons`, `/api/splits`.
...
La dashboard mostra **due rendimenti annualizzati complementari** (`DashboardKPIs.annualized_return` e `money_weighted_return`, entrambi None sotto ~90 giorni): il **TWR** (`_annualized_twr`, performance di mercato neutrale ai versamenti) e il **money-weighted / XIRR** (`_money_weighted_return` + helper `_xirr` per bisezione: acquisti negativi, vendite/dividendi positivi, valore attuale come flusso finale — segue il P&L reale). Possono differire anche di segno quando i versamenti sono concentrati in momenti favorevoli/sfavorevoli. Gli split non alterano i flussi XIRR (prezzo × quantità invariante).

**Split / raggruppamenti (`stock_splits`, `routers/splits.py`).** Operazione societaria dello **strumento** (come `price_history`): `(date, old_shares, new_shares)`, ratio = new/old (raggruppamento 10:1 → old 10, new 1, ratio 0.1; split 1:3 → ratio 3). `DashboardCalculator._split_factor(instrument_id, tx_date)` restituisce il prodotto dei ratio degli split **successivi** a `tx_date`; ogni loop di replay (open/closed positions, `_calc_realized_pnl`, serie valore e TWR) normalizza le transazioni precedenti a termini post-split: `qty × factor`, `prezzo ÷ factor` (controvalore/costo invariati). Così 500 azioni comprate + raggruppamento 10:1 + 50 vendute → posizione **chiusa** e P&L corretto. I prezzi Yahoo (`close`) sono già rettificati per split, quindi il valore storico resta coerente senza toccare `price_history`. CRUD in `/api/splits`; UI in `components/SplitSection.tsx` (pagina dettaglio strumento). La tabella è creata da `create_all` (nessuna migrazione dedicata).
